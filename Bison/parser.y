...
typedef struct {
    ...
    char expr_type[20];   	 	// Data type: INT, DOUBLE, CHAR, BOOLEAN, STRING
...
DeclaredVar declaredVar_table[MAX_EXPR];
int var_count = 0;
...
Var_check id_table[100];
int id_count = 0;


Extra_var ex_table[100];
int ex_count = 0;
...
assignment: VAR EQUAL expression QMARK | VAR EQUAL normal_type QMARK
{
...
expression: member_access | method_call | operations ;


operations:   operations PLUS operations  { $$ = $1 + $3 ; printf("Result is:  %lf \n", $$);   } 
			| operations MINUS operations { $$ = $1 - $3 ; printf("Result is:  %lf \n", $$);   }
			| operations MUL operations   { $$ = $1 * $3 ; printf("Result is:  %lf \n", $$);   }
			| operations DIV operations   { $$ = $1 / $3 ; printf("Result is:  %lf \n", $$);   }
			| num 
			;
